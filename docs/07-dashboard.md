...
**Stats to load:**
- Ideas: Count Query on `ideas` table with user_id filter
- Cryptos: Count Query on `cryptos` table with user_id filter
- Stadiums: Count Query on `stadiums` table with user_id filter
...
**List Items:**
- Links to Detail Pages (/ideas/{id})
...
**Actions to define:**
- "New Idea" → /ideas/new
- "New Crypto" → /cryptos/new
- "New Stadium" → /stadiums/new
