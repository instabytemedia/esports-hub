...
**Solution:** A blockchain-based social network that enables the trading of digital collectibles, participation in prediction markets, and engagement in AR-powered fantasy sports leagues
...
export function Logo({ size = "default" }: { size?: "sm" | "default" | "lg" }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`font-bold tracking-tight ${s.text}`}>Esports Hub</span>
    </div>
  );
}
```
